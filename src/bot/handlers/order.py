...
def compact_user_info(context: ContextTypes.DEFAULT_TYPE, is_order: bool = False) -> dict:
    user_info = {
    ...
    if is_order:
        order_info = {
            ...
            'services_price': context.user_data["services_price"],
            'total_price': context.user_data["total_price"],
        }
        full_info = {**user_info, **order_info}
...
async def ask_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if client:
        ...
        text = texts.ASK_REUSE_USER_SETTINGS_TEXT.format(**compact_user_info(context))
...
async def select_services(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "Готово" in text:
        # заглушка - изменить при получении схемы квартир
        context.user_data["entrance"] = 1
        context.user_data["floor"] = 1

        context.user_data['services_price'] = Pricing.services_price_message(services)
        context.user_data['total_price'] = Pricing.calculate_order_price(context.user_data["bags_number"], services)

        confirm_text = texts.ORDER_CHECKUP_TEXT.format(**compact_user_info(context, True))
...
async def payment_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.user_data["client_exists"]:
        client = await Repository.create_client(**compact_user_info(context))
    elif context.user_data["client_changed"]:
        client = await Repository.update_client(**compact_user_info(context))
    else:
        client = await Repository.get_client_by_telegram_id(telegram_id)
...
async def payment_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = texts.SUCCESS_TEXT.format(**compact_user_info(context, True))
